...
<<'INS status table'>>=
print(xtable(df, caption="Immigration statuses in the non-immigrant dataset", digits=c(0,0,0,2)), include.rownames=FALSE)
@
...
collapse1 <- rename(collapse1, c(cumsum="Cumumative total", cumPercent="Cumulative percent"))
@
...
x.rescale <- xtable(collapse1,label='tabrescaled',caption="Countries of birth with 20,000 or more status adjusters between 1999-2011, by geographical region. The number of status adjusters has been stratified by immigration status", digits=c(0,0,0,0,2,0,2))
...
<<'Visa type table'>>=
print(xtable(visacat, caption="Visa types with >=26,500 status adjusters between 1999-2011", digits=c(0,0,0,2)), include.rownames=FALSE)
@
...
visacat.yecai <- visacat.yecai[order(-visacat.yecai$Freq),]
visacat.yecai <- rename(visacat.yecai, c(Var1="Visa type", Freq="Number"))
visacat.yecai$Percent <- 100* visacat.yecai$Number/sum(visacat$Number)
@

<<'Yecai visa table'>>=
print(xtable(visacat.yecai, caption="Status adjustment applicants based on Yecai's visa classification", digits=c(0,0,0,2)), include.rownames=FALSE)
@
...
<<'COB and TB burden',fig.height=6,fig.width=15,fig.align='center',fig.cap="Country of birth of non-immigrant applicants for status adjustment, by TB burden">>=
require(ggplot2)
library(scales)
