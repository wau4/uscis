...
<<'INS status table'>>=
print(xtable(df, caption="INS statuses in the non-immigrant dataset", digits=c(0,0,0,2)), include.rownames=FALSE)
@
...
x.rescale <- xtable(collapse1,label='tabrescaled',caption="Countries of birth with 20,000 or more status adjusters between 1999-2011, by geographical region. The number of status adjusters has been stratified by immigration status")
...
p + geom_bar() + theme_bw() + coord_flip() +
  scale_y_continuous(labels=comma, breaks=c(20000, 100000, 200000, 300000, 400000, 500000, 600000)) +
  facet_wrap(~ tbburden.yecai, ncol=3)
