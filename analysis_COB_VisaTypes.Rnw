...
Using WHO's TB incidence data for 2010, we classified countries into high TB incidence ($>$100/100,000), medium TB incidence (20-100/100,000) and low TB incidence ($<$20/100,000).  Table 5 presents the number of status adjustment applicants by their country of birth, geographical region and WHO TB burden of their birth country.
...
ni.main <- merge(ni.main, tbwho.merge, by="COB")
...
require(gridExtra)
grid <- grid.arrange(p.hightb, p.medtb, p.lowtb, nrow=1, ncol=3, main=textGrob("Status adjustment applicants, by TB burden of their country of birth"), left=textGrob("Country of birth", rot=90))
pdf("WHOTB_COB.pdf", width=20, height=6)
plot(grid)
dev.off()
@

<<'print plot'>>=
![](C:/Users/wau4/Desktop/uscis/uscis/WHOTB_COB.pdf)
@
...
hightb <- subset(ni.main, tbburden.yecai=="high (TB incidence>100/100,000)", drop=TRUE) 
hightb$tbburden.yecai <- factor(hightb$tbburden.yecai)
hightb$COB <- factor(hightb$COB)

mediumtb <- subset(ni.main, tbburden.yecai=="medium (TB incidence >15 to <=100/100,000)", drop=TRUE) 
mediumtb$tbburden.yecai <- factor(mediumtb$tbburden.yecai)
mediumtb$COB <- factor(mediumtb$COB)

lowtb <- subset(ni.main, tbburden.yecai=="low (TB incidence<=15/100,000)", drop=TRUE) 
lowtb$tbburden.yecai <- factor(lowtb$tbburden.yecai)
lowtb$COB <- factor(lowtb$COB)
@

<<'Generating individual plots-by Yecai TB',eval=FALSE>>=
#high tb
aggdata.hightb <- aggregate(hightb, by=list(hightb$COB), FUN=length)
keepvars <- names(aggdata.hightb) %in% c("Group.1", "COB")
aggdata.hightb <- aggdata.hightb[keepvars]
...
<<'Generating combined plot-by TB',fig.height=6,fig.width=20,fig.align='center',fig.cap="Country of birth of non-immigrant applicants for status adjustment, by TB burden",eval=FALSE>>=
require(gridExtra)
grid.arrange(p.hightb.y, p.medtb.y, p.lowtb.y, nrow=1, ncol=3, main=textGrob("Status adjustment applicants, by Yecai TB burden of their country of birth"), left=textGrob("Country of birth", rot=90))
@
\FloatBarrier
